...
SetTrust::doApply()
{
    ...
    bool const bHigh = ctx.tx.getAccountID(sfAccount) > uDstAccountID;
        ...
        ctx.tx.getAccountID(sfAccount) == uDstAccountID)
    {
        return trustDelete(
            ctx.view(),
            ctx.view().peek(keylet::line(ctx.tx.getAccountID(sfAccount), uDstAccountID, currency)),
    ...
    saLimitAllow.setIssuer(ctx.tx.getAccountID(sfAccount));
    ...
    if (sleRippleState)
    {
        ...
        auto const& uLowAccountID = !bHigh ? ctx.tx.getAccountID(sfAccount) : uDstAccountID;
        auto const& uHighAccountID = bHigh ? ctx.tx.getAccountID(sfAccount) : uDstAccountID;
    ...
    else
    {
        ...
        auto const k = keylet::line(ctx.tx.getAccountID(sfAccount), uDstAccountID, currency);
