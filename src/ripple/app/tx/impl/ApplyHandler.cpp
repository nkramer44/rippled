...
ApplyHandler::ApplyHandler(ApplyContext& applyCtx, doApplyPtr doApplyFn)
    : ctx(applyCtx), doApply_(doApplyFn)
{
}
...
ApplyHandler::consumeSeqProxy(SLE::pointer const& sleAccount)
{
    ...
    return ticketDelete(
        ctx.view(), ctx.tx.getAccountID(sfAccount), getTicketIndex(ctx.tx.getAccountID(sfAccount), seqProx), ctx.journal);
}
...
ApplyHandler::apply()
{
    ...
    return doApply_(ctx, mPriorBalance, mSourceBalance);
}
